...
export default function DirectoryError({ reset }: { error: Error; reset: () => void }) {
  return <section role="alert" className="space-y-3 rounded-lg border border-border bg-card p-5">
    <h2 className="font-semibold">Could not load staff records</h2>
    ...
    <div className="flex flex-wrap items-center gap-3"><Button onClick={reset}>Retry</Button><Link href="/staff" className="text-sm text-primary">Back to staff</Link></div>
  </section>
}
