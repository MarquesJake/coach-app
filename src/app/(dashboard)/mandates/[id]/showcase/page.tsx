...
export default async function TottenhamShowcase({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div id="tottenham-showcase" className="mx-auto max-w-6xl space-y-12 pb-16">
    ...
    <header className="case-cover rounded-2xl border border-border bg-card px-6 py-8 sm:px-10 sm:py-12">
      ...
      <p className="mt-6 max-w-3xl text-xs leading-6 text-muted-foreground">Public facts are linked to sources. Worked assessments, budgets and plans are Gaffa working material ahead of interviews and references. This is a dated presentation edition; live research records may subsequently change.</p>
    </header>
    ...
    <section id="brief">
      <Heading number="01" title="Support now. Prepare properly." subtitle="Board brief · current position ahead of the mandate reviews."/>
      ...
      <div className="case-grid mt-8 grid gap-4 sm:grid-cols-3">{[['Essential','Work with the inherited squad; explain a credible first month; accept clear decision rights.'],['Preferred','Demonstrable player improvement; adaptable attacking solutions; effective multidisciplinary work.'],['Flexible','Exact formation, coaching vocabulary and the sequence of longer-term tactical changes.']].map(([label,body])=><div key={label} className="rounded-xl border border-border p-5"><p className="case-label">{label} · mandate requirement</p><p className="case-copy mt-3">{body}</p></div>)}</div>
    </section>

    <section id="field" className="case-section">
      <Heading number="02" title="A field with a purpose." subtitle="Research order. These are different routes to test, not a ranked appointment shortlist."/>
    ...
    <section id="dossiers" className="case-section">
      <Heading number="03" title="The case behind each name." subtitle="All nine areas from the assessment framework. Public baselines are separated from worked hypotheses, tests and reasons for caution. Historical results now inform each case; the 63 working sections are hypotheses to test, not completed evidence."/>
      <div className="space-y-5">{dossier.coaches.map((c,index)=><details id={`coach-${c.key}`} key={c.id} className="rounded-xl border border-border bg-card p-5 sm:p-7">
        ...
        <div className="mt-7 space-y-7">
          ...
          <div className="case-disclaimer text-xs font-semibold text-primary">Working assessment below · interviews and references to follow</div>
    ...
    <section id="delivery" className="case-section">
      <Heading number="04" title="From a good story to a defensible decision." subtitle="Work programme · proposed responsibilities and dates."/>
