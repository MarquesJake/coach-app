...
export default async function TottenhamShowcase({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div id="tottenham-showcase" className="mx-auto max-w-6xl space-y-12 pb-16">
    ...
    <header className="case-cover rounded-2xl border border-border bg-card px-6 py-8 sm:px-10 sm:py-12">
      ...
      <p className="case-copy mt-6 max-w-2xl">Back De Zerbi, or line up a successor? Six alternatives studied against him — the football case, what it would take to make it happen, and what could change our view.</p>
    ...
    <section id="brief">
      <Heading number="01" title="Support now. Prepare properly." subtitle="Board brief · where we stand before the reviews."/>
      <div className="case-grid grid gap-8 lg:grid-cols-[1.4fr_1fr]">
        <div className="space-y-6"><Note title="The situation"><p>One point and no goals from the first three league games. That needs looking at, but it is a small sample — and the 5–1 cup win and a tighter display at Forest count too.</p><Sources ids={['spurs-bees','spurs-newcastle','spurs-forest','spurs-cup']}/></Note>
          <Note title="Our position"><p>Back De Zerbi for now with a clear improvement plan, and line up the alternatives. McKenna is first to research, with Hoeneß the strongest comparison. Knutsen is a longer-term option. Silva, Terzić and Farioli are there to test whether we are heading in the right direction.</p></Note>
    ...
    <section className="case-section"><Heading number="06" title="The first 90 days." subtitle="A plan to test with any coach — and a fair yardstick for sticking with De Zerbi."/><div className="case-grid grid gap-5 md:grid-cols-3">{dossier.plan.map(([days,title,body,review])=><article key={days} className="rounded-xl border border-border p-6"><p className="case-label">{days}</p><h3 className="mt-3 font-serif text-2xl">{title}</h3><p className="case-copy mt-4">{body}</p><div className="mt-5 border-t border-border pt-4"><Note title="Review question">{review}</Note></div></article>)}</div></section>
