...
export function EloTrends() {
  ...
  return <section id="elo" className="case-section space-y-6" aria-labelledby="elo-title">
    <header><p className="case-label">Evidence / ClubElo</p><h2 id="elo-title" className="mt-2 font-serif text-3xl font-medium tracking-tight">How strong did the team become?</h2><p className="case-copy mt-3 max-w-3xl">Follow the club’s rating through each coach’s spell: where they started, how far they climbed and what happened after the peak. Elo allows for the strength of the opposition. It rates the team, not the coach.</p></header>
    ...
    <details className="rounded-xl border border-border p-5"><summary className="flex items-center justify-between gap-4 text-sm font-medium">How to read this<ChevronDown className="case-chevron h-4 w-4"/></summary><div className="case-copy mt-5 space-y-3">
      <p>Higher Elo means a stronger team. ClubElo allows for opposition strength, home advantage, winning margin and differences between leagues, and it includes European games — unlike the league-only results above.</p>
      <p>A rise under a coach does not prove he caused it — recruitment, money and the wider club matter too. Spells differ in length and starting point, so this is not a league table of coaches.</p>
      ...
      <p>The starting figure is the last rating on or before he arrived; if there isn’t one, we use the first rating during his spell and mark the change as partial. The end figure is the last rating on or before he left. Every figure shows its date. The peak is the highest rating in that period.</p>
      <p>The published history can be revised, and a gap between ratings does not mean the team stayed the same. “Latest” is the latest rating saved for this edition, not a live feed. Not every earlier club or season is covered.</p>
...
function SpellChart({ spell }: { spell: Spell }) {
  ...
  return <div className="space-y-6 pt-6" aria-label="Published ClubElo trend">
    ...
    {summary.discontinuous && <p className="text-xs text-amber-700">There is a break in this history, so the separate lines should not be read as continuous.</p>}
    ...
    <figure aria-label={`${spell.club} published Elo ratings during ${spell.coach}'s spell`}>
      ...
      <p className="mt-3 text-[11px] leading-5 text-muted-foreground">Elo points · dashed lines mark arrival and departure · saved ratings, not a forecast. Includes up to 90 days either side of the spell.</p>
    </figure>
