...
import { verifiedExampleForCoach } from '@/lib/demo/verified-examples'
...
export default async function CoachOverviewPage(props: { params: Promise<{ id: string }> }) {
  ...
  if ([externalProfileRes, stintsRes, shortlistRes, longlistRes, portalRes, materialsRes, claimsRes].some(result => result.error)) throw new Error('Full coach record could not be loaded. Reload before relying on this view.')
  ...
  const publicSnapshot = verifiedExampleForCoach(coach.id)
                ...
                <li key={`${entry.relation}-${entry.id}`} className="rounded-lg border border-border bg-surface/40 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <Link href={`/mandates/${entry.mandate.id}/decision`} className="block truncate text-sm font-semibold text-foreground hover:underline">
                    ...
                    <Link href={`/mandates/${entry.mandate.id}/decision`} className="shrink-0 text-xs text-muted-foreground transition-colors hover:text-foreground">
                      Open
                    </Link>
                  </div>
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          {text(coachRecord.contract_notes) && (
            <div className="mt-3 rounded-lg border border-border bg-surface/40 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Contract notes</p>
