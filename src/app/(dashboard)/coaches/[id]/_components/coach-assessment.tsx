...
export async function CoachAssessment({ coachId, areas, data }: { coachId: string; areas?: CriterionKey[]; data?: {questions: CoverageQuestion[]; findings: CoverageFinding[]} }) {
  ...
  if (!loaded) {
    ...
    if (research.error || claims.error) return <section className="gaffa-panel"><p role="alert" className="text-sm">The coach assessment could not be loaded. Reload before relying on its coverage.</p></section>
  ...
  return <section className="gaffa-panel">
    <div className="mb-5"><p className="gaffa-eyebrow mb-2">{areas ? 'Supporting research' : 'The assessment'}</p><h2 className="gaffa-panel-heading">{areas ? 'What the research tells us' : 'Coach assessment at a glance'}</h2><p className="gaffa-description mt-2">General coach research: recorded conclusions, the accounts behind them and what still needs checking. These are internal research views, not approved recommendations.</p></div>
      ...
      return <details key={area.key} open={!!areas && area.answers.length > 0} className="gaffa-disclosure py-4">
        ...
        <div className="mt-4 space-y-4 border-l-2 border-primary/30 pl-4">
          {area.answers.slice(0,2).map(answer => <article key={answer.id} className="space-y-2">
            ...
            {answer.needsReview && <p className="text-xs font-medium text-amber-700 dark:text-amber-300">Source review needed: some linked findings are unverified, outdated, examples or unavailable.</p>}
          ...
          <div className="rounded-lg bg-muted/50 p-3"><h3 className="text-xs font-semibold">Evidence still to check</h3>{area.missingMethods.length ? <ul className="mt-2 space-y-1 text-xs text-muted-foreground">{area.missingMethods.map(method => <li key={method}>{methodLabel(method)} — not recorded against a supported answer in this area.</li>)}</ul> : <p className="mt-2 text-xs text-muted-foreground">The priority methods are recorded. Review their quality, independence and relevance before drawing a conclusion.</p>}<p className="mt-2 text-xs text-muted-foreground">Coverage reflects linked research only; evidence may exist elsewhere and still need connecting.</p></div>
