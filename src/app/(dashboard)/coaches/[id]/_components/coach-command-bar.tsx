...
export function CoachCommandBar({coachId, coach, onWatchlist=false, evidenceLabel}: {coachId:string; coach:Record<string,unknown>; onWatchlist?:boolean; evidenceLabel:string}) {
 ...
 const name=canonicalCoachName(coachId, coach.name ? String(coach.name) : null)
 ...
 return <header className="mb-6 min-w-0 pt-2">
  <div className="flex flex-wrap items-start justify-between gap-4">
    <div className="min-w-0"><p className="gaffa-eyebrow mb-2">Coach profile</p><h1 className="break-words text-3xl font-semibold tracking-tight sm:text-4xl">{name}</h1><p className="mt-2 text-sm text-muted-foreground">{employment ? `${employmentLabel(employment)} · checked ${employment.checkedAt}` : recordedRole ? `Old provider record: ${recordedRole} — not checked, may be out of date` : 'Current role not checked'}</p></div>
  ...
  <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
    <div className="flex max-w-xl flex-wrap items-center gap-2 text-xs text-muted-foreground"><span className="gaffa-badge">{evidenceLabel}</span><span>{employment?.status === 'employed' ? 'In a job — interest and release terms unknown' : employment?.status === 'unattached' ? 'No club on record — interest unknown' : 'Availability not confirmed'}</span></div>
