...
export function ResearchForm({ coachId, row, findings, mandates, initialDomain, initialMandate, initialTemplate }: { coachId: string; row?: Question; findings: FindingOption[]; mandates: { id: string; name: string }[]; initialDomain?: string; initialMandate?: string; initialTemplate?: ResearchTemplate }) {
  ...
  return <form className="space-y-4" onSubmit={event => {
    ...
    start(async () => {
      ...
      } catch { setMessage({ error: 'Couldn’t confirm it saved. Your entries are kept — refresh and try again.' }) }
    })
  }}>
    ...
    <details open={Boolean(row?.answer)} className="rounded-lg border border-border p-4"><summary className="cursor-pointer text-sm font-medium">Assessment and sources</summary><div className="mt-4 space-y-4">
      <fieldset><legend className="text-sm font-medium">Evidence methods used</legend><p className="mt-1 text-xs text-muted-foreground">Only tick the methods behind the findings you link below. Planned interviews and visits go in “How we’ll check”.</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{EVIDENCE_METHODS.map(m => <label key={m.key} className="flex gap-2 text-sm"><input type="checkbox" name="evidence_methods" value={m.key} defaultChecked={row?.evidence_methods?.includes(m.key) ?? false}/>{m.key === 'ai_generated' ? 'AI-assisted synthesis (requires source review)' : m.label}</label>)}</div></fieldset>
      ...
      <label className="block text-sm">Anyone who sees it differently, and the coach’s response<textarea name="counter_evidence" maxLength={8000} defaultValue={row?.counter_evidence} rows={3} className={control} placeholder="Preserve the strongest counterargument and unresolved differences." /></label>
      <fieldset><legend className="text-sm font-medium">Linked findings</legend><p className="mb-2 text-xs text-muted-foreground">Link findings that back up or challenge your view. Saving doesn’t mean it can be shared.</p>{findings.length ? findings.map(f => <label key={f.id} className="my-2 flex gap-3 rounded border border-border p-3 text-sm"><input type="checkbox" name="evidence_claim_ids" value={f.id} defaultChecked={row?.evidence_claim_ids.includes(f.id)} /><span>{f.claimed_value}<small className="mt-1 block text-muted-foreground">{f.source_name || 'Source missing'} · {evidenceLabel(evidenceStatus(f))} · {f.occurred_at?.slice(0,10) || 'Undated'}</small></span></label>) : <p className="text-sm text-muted-foreground">No findings yet. Save the question, then log a conversation or source in Intelligence.</p>}</fieldset>
    </div></details>
