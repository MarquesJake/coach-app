...
export default async function ResearchPage({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<ResearchParams> }) {
  ...
  if (requestedMandate && !initialMandate) return <p role="alert">This appointment could not be loaded. Return to the appointment and reopen research; no general question will be created by default.</p>
  ...
  return <div className="space-y-5">
    {initialMandate && <Link className="text-sm text-primary underline" href={href(`/mandates/${initialMandate}/decision`)}>Back to appointment overview</Link>}
    <section className="flex flex-wrap items-start justify-between gap-4"><div><p className="gaffa-eyebrow mb-2">Build the evidence</p><h2 className="text-2xl font-semibold tracking-tight">Research</h2><p className="gaffa-description mt-2 hidden sm:block">Ask what could change the decision. Link sources to your answer and record any conflicting accounts.</p><div className="mt-4 hidden flex-wrap gap-4 sm:flex"><Link className="gaffa-link" href={captureHref(`/intelligence/conversations?coach=${id}`)}>Record conversation</Link><Link className="gaffa-link" href={captureHref(`/intelligence/inbox?coach=${id}`)}>Add source material</Link><Link className="gaffa-link" href={href(`/coaches/${id}/intelligence`)}>Review findings</Link></div></div><Link className="gaffa-action gaffa-action-primary" href="#new-question"><Plus className="h-4 w-4"/>New question</Link></section>
    {(questions.error || claims.error || mandates.error) && <p role="alert" className="rounded border border-destructive p-4 text-sm">Some research, findings or appointment contexts could not be loaded. Reload before making changes.</p>}
    <div className="flex flex-wrap gap-3 text-sm"><strong>{general || !initialMandate ? 'General coach research' : `Appointment research: ${mandateOptions.find(m => m.id === initialMandate)?.name}`}</strong>{initialMandate && <Link className="underline" href={researchHref(`/coaches/${id}/research?scope=${general ? 'appointment' : 'general'}`, { ...context, question: undefined })}>{general ? 'This appointment' : 'Separate general research'}</Link>}<details><summary className="cursor-pointer underline">Switch appointment</summary>{mandateOptions.map(m => <Link className="block py-2 underline" key={m.id} href={researchHref(`/coaches/${id}/research`, { coach: id, mandate: m.id })}>{m.name}</Link>)}</details></div>
    <p className="text-xs text-muted-foreground">General research is reusable context. It is not an appointment conclusion until explicitly assessed for that brief.</p>
