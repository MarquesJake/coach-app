...
export default async function CoachOverviewPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<ResearchParams> }) {
  ...
  return <div className="space-y-7">
    {(claims.error || research.error) && <p role="alert" className="rounded-lg border p-4">Some evidence or research could not be loaded. Refresh before making a decision.</p>}
    <section>
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="gaffa-panel-heading">Coach overview</h2><p className="gaffa-description mt-1">What we know, what supports it and what still needs checking.</p></div><Link className="gaffa-link inline-flex items-center gap-2" href={`${base}/record`}>Full profile<ArrowUpRight className="h-4 w-4"/></Link></div>
      ...
      <details className="text-xs text-muted-foreground"><summary className="w-fit cursor-pointer underline decoration-border underline-offset-4">About these figures</summary><p className="mt-2 max-w-3xl leading-relaxed">A checked source does not confirm every claim or establish suitability. {evidence.illustrative} example records excluded; {evidence.stale} need updating.</p></details>
    </section>
