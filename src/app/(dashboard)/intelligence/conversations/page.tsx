...
export default async function ConversationsPage({ searchParams }: { searchParams?: { coach?: string; contact?: string } }) {
  ...
  return <div className="space-y-4"><ConversationCaptureClient organizationId={organizationId} contacts={contacts ?? []} coaches={coaches ?? []} defaultCoachId={selectedCoachId} defaultContactId={selectedContactId} /><div className="overflow-x-auto border border-border bg-card"><table className="w-full min-w-[820px] text-left text-sm"><thead className="border-b border-border bg-muted/40 text-xs text-muted-foreground"><tr><th className="px-4 py-3 font-medium">Conversation</th><th className="px-4 py-3 font-medium">Source</th><th className="px-4 py-3 font-medium">Coach</th><th className="px-4 py-3 font-medium">Findings</th><th className="px-4 py-3 font-medium">State</th></tr></thead><tbody className="divide-y divide-border">{(sessions ?? []).map((session: Record<string, any>) => { const counts = claimCounts.get(session.id) ?? { total: 0, accepted: 0 }; return <tr key={session.id}><td className="px-4 py-3"><Link href={`/intelligence/review?session=${session.id}`} className="font-medium hover:text-primary">{String(session.title)}</Link><p className="text-xs text-muted-foreground">{new Date(session.occurred_at).toLocaleString('en-GB')} · {formatEnumLabel(String(session.intake_method))}</p></td><td className="px-4 py-3 text-muted-foreground">{String(contactMap.get(session.contact_id) || 'Analyst source not linked')}</td><td className="px-4 py-3">{session.coach_id ? <Link href={`/coaches/${session.coach_id}/intelligence`} className="hover:text-primary">{String(coachMap.get(session.coach_id) || 'Coach')}</Link> : <span className="text-muted-foreground">General</span>}</td><td className="px-4 py-3 text-muted-foreground">{counts.accepted}/{counts.total} reviewed</td><td className="px-4 py-3 text-xs text-muted-foreground">{formatEnumLabel(String(session.processing_status))}</td></tr>})}{!(sessions ?? []).length && <tr><td colSpan={5} className="px-4 py-12 text-center text-muted-foreground">No conversations captured yet.</td></tr>}</tbody></table></div></div>
}
