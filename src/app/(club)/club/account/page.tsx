...
export const metadata = { title: 'Account · Club' }
...
export default async function ClubAccountPage() {
  ...
  const [membershipsRes, identitiesRes, { data: { user } }, organizationAccess] = await Promise.all([
    ...
    getOrganizationAccessProfile(context.userId),
  ])
  if (membershipsRes.error || identitiesRes.error) throw new Error('Club account information could not be loaded.')
        ...
        <div className="divide-y divide-border/60">
          ...
          {(memberships ?? []).map((membership) => {
            ...
            const isInternalPreview =
              membership.user_id === user?.id && organizationAccess.hasActiveInternalAccess
            return (
              <div key={membership.id} className="grid gap-2 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_160px_110px] sm:items-center">
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {identity?.display_name ?? (membership.user_id === user?.id ? user.email : 'Invited club member')}
                  </p>
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    {identity?.position_title ?? (membership.user_id === user?.id ? 'Current account' : 'Organisation seat')}
                  </p>
                </div>
                <span className="text-xs capitalize text-muted-foreground">{membership.role.replaceAll('_', ' ')}</span>
                <span className={`text-xs font-medium ${
                  membership.status === 'active' && identity?.onboarding_completed_at
                    ? 'text-emerald-700'
                    : isInternalPreview
