...
export default async function ClubDossierDetailPage(
  ...
  return (
    <div className="mx-auto max-w-[1060px]">
      ...
      {searchParams.ordered && <div className="mt-4 rounded-md border border-emerald-700/20 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">Request sent. Gaffa will be in touch privately to confirm what’s included and the next step.</div>}
      {searchParams.error && <div className="mt-4 rounded-md border border-red-700/20 bg-red-50 px-4 py-3 text-sm text-red-900">The request didn’t go through. Check what you’ve entered and your access.</div>}
      ...
      <div className="mt-6 grid gap-6 [&>*]:min-w-0 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-5 break-words">
          {release.canViewMaterials && materials.length === 0 && <p role="status" className="rounded border border-border bg-card p-4 text-sm">Access is active, but no released files were returned. <Link href="/club/account#access-help" className="underline">Ask your Gaffa contact which files have been released.</Link></p>}
        ...
        <aside>
          <div className="sticky top-6 rounded-md border border-border bg-card p-5">
            ...
            <p className="mt-2 text-xs leading-5 text-muted-foreground">Gaffa agrees what’s included and the terms with you privately, then checks the coach’s permission before releasing anything.</p>
            ...
            {!order && (['owner', 'admin', 'club_owner', 'club_director'].includes(context.membershipRole) ? <DossierRequestForm offerId={offer.id} /> : <p className="mt-4 text-sm text-muted-foreground">This request has to come from a club director or owner. <Link href="/club/account#access-help" className="underline">Access help</Link></p>)}

            {order && <div className="mt-5 space-y-3"><div className="rounded-md border border-border bg-background p-3"><p className="text-[10px] font-semibold uppercase text-muted-foreground">Request status</p><p className="mt-1 text-sm font-semibold text-foreground">{release.label}</p></div>{release.state === 'active' ? <div className="flex items-start gap-2 rounded-md border border-emerald-700/20 bg-emerald-50 p-3 text-xs leading-5 text-emerald-950"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0" />Gaffa has approved the release. The files are unlocked on this page.</div> : release.state === 'expired' ? <p className="rounded-md border border-amber-700/20 bg-amber-50 p-3 text-xs leading-5 text-amber-950">Your access has ended and the files are locked. Ask Gaffa to renew it.</p> : release.state === 'revoked' ? <p className="rounded-md border border-red-700/20 bg-red-50 p-3 text-xs leading-5 text-red-950">Access has been withdrawn and the files are locked. Contact Gaffa if the board still needs them.</p> : release.state === 'declined' || release.state === 'cancelled' ? <p className="text-xs leading-5 text-muted-foreground">This request is closed. Contact Gaffa if you need something different.</p> : <p className="text-xs leading-5 text-muted-foreground">Gaffa is checking what can be shared and getting the coach’s permission. You’ll see the files once the release is approved.</p>}</div>}
          </div>
        </aside>
      </div>
    </div>
