...
const stages = [
  {
    key: 'appointment',
    label: 'Appointment',
    description: 'Why you’re looking, the timing, and who can know.',
  },
  {
    key: 'football',
  ...
  },
  {
    ...
    description: 'The people, the development expectations and the culture around the job.',
  },
  {
    key: 'feasibility',
    label: 'Feasibility',
...
const footballFields = [
  ['appointment_context', 'Appointment context', 'Why the club might make a change, the timing, what would trigger it and what must stay confidential.'],
  ['football_identity', 'Football identity', 'The football you won’t compromise on, and where the coach must adapt to the league and squad.'],
  ['in_possession_requirements', 'In possession', 'Playing out, moving the ball forward, creating chances, winning territory and how direct you’re happy to be.'],
  ['out_of_possession_requirements', 'Out of possession', 'When to press, how to defend as a block, cover behind the ball and the physical demands.'],
...
const squadFields = [
  ['squad_context', 'Squad context', 'Ages, contracts, big personalities, gaps and the players the next coach must improve.'],
  ['player_development_priorities', 'Player development', 'The academy pathway, growing player value and what you expect for individual development.'],
  ['leadership_and_culture', 'Leadership and culture', 'Working with the board, how he communicates, leading the staff and handling pressure.'],
] as const

const feasibilityFields = [
  ['budget_parameters', 'Financial parameters', 'Compensation, salary, staff package and any limits on recruitment.'],
  ['availability_timeline', 'Availability and timeline', 'Preferred start date, how long the process can take and any limits on who we can speak to.'],
  ...
  ['process_requirements', 'Decision process', 'Interviews, presentations, references, background checks and what the board needs to see.'],
  ['confidentiality_notes', 'Confidentiality protocol', 'Who can know, when we can make contact, and who to go to if something comes up.'],
] as const
  ...
  return (
    <div className="mx-auto max-w-[1000px]">
      <div className="border-b border-border pb-5">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Tell us the football problem and the club situation in your own words. This becomes the backbone of the search — it isn’t a job advert.</p>
      </div>
      <section className="mt-4 rounded-md border border-border bg-card p-4 text-sm">
        ...
        <p className="mt-2 text-xs text-muted-foreground">Sending the brief doesn’t commit either side, and we won’t contact coaches without your go-ahead.</p>
      </section>
      ...
      {searchParams.error && <div role="alert" className="mt-4 rounded-md border border-red-700/20 bg-red-50 px-4 py-3 text-sm text-red-900">{searchParams.error === 'agreed' ? 'This brief is agreed or linked to a mandate. Its wording has not changed. Use the amendment request below.' : 'The brief didn’t save. Check the required fields and your access, then refresh and try again.'}</div>}
      {isAgreed && <p className="mt-4 rounded border border-border bg-card p-4 text-sm">{amendmentResult.error ? 'Original wording — the latest agreed version couldn’t be confirmed.' : `Agreed brief · version ${agreed.version}.`} Request an amendment below when requirements change. The original wording and all decisions are retained.</p>}
        ...
        <div className="mt-4 flex flex-col gap-3 border border-emerald-700/20 bg-emerald-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-2">
            ...
            <p className="text-sm text-emerald-950">
              Next: Gaffa reviews the brief, goes through any open points with you and prepares the first list of names.
            </p>
          </div>
      ...
      {amendmentResult.error && <p role="alert" className="mt-4 rounded border border-red-300 p-4 text-sm">Changes to the brief didn’t load, so the wording below may be out of date. Refresh before relying on it or asking for a change.</p>}
      ...
      >
        ...
        <FieldRows rows={[footballFields[0], ['confidentiality_notes', 'Confidentiality protocol', 'Who can know, when we can make contact, and who to go to if something comes up.']]} brief={brief} canEdit={canEdit} />
        </div>
      ...
      {isAgreed && !amendmentResult.error && <section className="mt-8 space-y-5 border-t border-border pt-6">
        ...
        {amendments.some(a => a.status === 'pending') ? <p className="text-sm text-muted-foreground">Gaffa is looking at your last change. Wait for our reply before asking for another.</p> : ['owner', 'admin', 'club_owner', 'club_director'].includes(context.membershipRole) && <details className="rounded border border-border bg-card p-4">
