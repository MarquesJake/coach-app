...
test('incumbent role is limited to the Tottenham mandate and excluded from decision cards', () => {
  ...
  assert.match(index, /!isCurrentManagerBenchmark\(mandateId, r.coach_id\)/)
  assert.match(index, /Current manager — benchmark only, not a successor/)
...
test('dated Tottenham dossier preserves all seven profiles but treats De Zerbi as benchmark', () => {
  ...
  assert.match(showcase, /The ranking from the brief puts/)
  assert.match(showcase, /De Zerbi is not a successor candidate/)
})
