...
export async function sendInvitationEmail(
  ...
  if (process.env.DEMO_MODE === 'true') return { status: 'not_configured' }
    ...
    const { data, error } = await resend.emails.send(
      {
        ...
        subject: `Your Gaffa ${destination} invitation`,
        html: `
          <div style="background:#f6f7f5;padding:32px 16px;font-family:Arial,sans-serif;color:#17201c">
            <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #dfe5e1;padding:32px">
              <p style="margin:0 0 8px;font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#486055">Gaffa</p>
              <h1 style="margin:0 0 16px;font-family:Georgia,serif;font-size:26px;line-height:1.25">Secure invitation</h1>
              <p style="font-size:15px;line-height:1.65">You have been invited to the Gaffa ${destination} for <strong>${organizationName}</strong> with <strong>${invitedRole}</strong> access.</p>
              ...
              <p style="font-size:12px;line-height:1.6;color:#7b8982">If you were not expecting this invitation, ignore this email or contact Gaffa directly.</p>
            </div>
          </div>
        `,
      },
