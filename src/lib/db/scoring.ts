...
export async function getLatestCoachScore(
  ...
  const { data: scoreRow, error: scoreErr } = await (supabase as any).from('coach_scores').select('id, coach_id, scoring_model_id, overall_score, tactical_score, leadership_score, recruitment_score, risk_score, media_score, confidence_score, computed_at').eq('coach_id', coachId).order('computed_at', { ascending: false }).limit(1).maybeSingle()
