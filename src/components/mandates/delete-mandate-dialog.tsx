...
export function DeleteMandateDialog({ mandateId, name, onClose, onDeleted, action = deleteMandateAction }: {
  ...
  return <dialog ref={dialogRef} aria-labelledby="delete-mandate-title" aria-describedby="delete-mandate-description"
    ...
    <p id="delete-mandate-description" className="mt-3 text-sm leading-6">Use this to remove a test or unwanted run. This permanently deletes the mandate and its linked shortlist, assessments, interview and reference records. It cannot be undone. Club and coach profiles remain.</p>
    <p className="mt-2 text-sm text-muted-foreground">Mandates linked to club briefs, dossiers or confidential access history are protected. Close those mandates to retain the record.</p>
