...
export async function ResearchQueue({mandateId, coachId, expanded = false}: {mandateId?: string; coachId?: string; expanded?: boolean}) {
 ...
 {error ? <p role="alert" className="text-sm">Research queue unavailable. Refresh before relying on this view.</p> : rows.length ? <>{rows.slice(0,expanded ? 50 : 3).map(renderQuestion)}{!expanded && rows.length>3 && <details className="gaffa-disclosure border-t border-border pt-4"><summary className="flex items-center justify-between text-sm font-medium text-primary">{rows.length-3} more questions<ChevronDown className="disclosure-chevron h-4 w-4"/></summary><div className="mt-3">{rows.slice(3).map(renderQuestion)}</div></details>}{rows.length===50 && <p className="mt-3 text-xs text-muted-foreground">Showing the next 50 open questions. Open a coach’s research for their full record.</p>}</> : <p className="text-sm text-muted-foreground">No open research questions recorded{mandateId ? ' for this appointment' : ''}. Add a question from a coach’s Research page.</p>}
 </section>
}
