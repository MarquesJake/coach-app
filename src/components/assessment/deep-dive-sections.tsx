...
import type { DeepDive, FinalEvaluation, Level, XgSplit } from '@/lib/assessment/deep-dive'
...
export function DeepDiveSource() {
  return <p className="mt-3 text-[10px] text-muted-foreground/70">Source: Gaffa data model · indicative. Licensed provider data (xG, physical, market value) replaces these figures once connected.</p>
}
...
export function PerformanceDeepDive({ d }: { d: DeepDive }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 sm:grid-cols-2 print:grid-cols-2"><XgRows label="xG for" x={p.xgFor} /><XgRows label="xG against" x={p.xgAgainst} /></div>
...
export function MatchManagementDeepDive({ d }: { d: DeepDive }) {
  return <div className="space-y-3"><Stats items={d.matchManagement.stats} /><p className={body}>{d.matchManagement.notes}</p><DeepDiveSource /></div>
}
...
export function PersonalityDeepDive({ d }: { d: DeepDive }) {
  return <div className="grid gap-3 sm:grid-cols-2 print:grid-cols-2">{d.personality.map((s, i) => <div key={s.heading} className="border-l-2 border-emerald-500/50 pl-3 print:break-inside-avoid"><p className="text-xs font-semibold text-foreground">{i + 1}. {s.heading}</p><p className={body}>{s.text}</p></div>)}</div>
}
...
export function AreaDeepDive({ area, d }: { area: string; d: DeepDive }) {
  ...
  return <div className="deep-dive mt-3 rounded-lg border border-border bg-card/40 p-4"><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-700 dark:text-emerald-400">{entry.title}</p>{entry.render(d)}</div>
}
...
export function FinalEvaluationSection({ e, verdict, confidence }: { e: FinalEvaluation; verdict?: string | null; confidence?: number | null }) {
  ...
  return <div className="deep-dive space-y-5">
    ...
    <div className="grid gap-4 sm:grid-cols-[1fr_2fr] print:grid-cols-[1fr_2fr]">
      <div className="rounded-lg border border-border p-4 text-center"><p className={label}>Probability of success</p><p className="mt-1 text-3xl font-bold tabular-nums text-foreground">{e.probabilityOfSuccess}%</p><div className="mt-2"><Bar value={e.probabilityOfSuccess} /></div></div>
      <div className="rounded-lg border border-border p-4"><p className={label}>Final recommendation</p><p className="mt-1 text-lg font-semibold text-foreground">{verdict ?? 'Not decided'}{confidence != null && <span className="ml-2 text-sm font-normal text-muted-foreground">{confidence}% confidence</span>}</p><p className={`${body} mt-1`}>{e.probabilityRationale}</p></div>
    </div>
    <p className="text-[10px] text-muted-foreground/70">Probability of success is Gaffa’s indicative model — it combines the nine area scores, the fit with the brief and how realistic the deal is. It supports the board’s judgement; it doesn’t replace it.</p>
  </div>
}
