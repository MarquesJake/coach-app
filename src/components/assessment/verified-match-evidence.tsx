...
export function VerifiedMatchEvidence({ coachName, apiIds, hideMissing = false }: { coachName: string; apiIds?: readonly number[]; hideMissing?: boolean }) {
  ...
  return <section aria-label="Verified match data" className="my-6 rounded-xl border border-emerald-600/40 bg-card p-5 sm:p-6">
    ...
    <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-lg font-semibold">Verified match data</h2><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-950 dark:bg-emerald-950 dark:text-emerald-200">Real match records · API-Football</span></div>
    <p className="mt-2 text-sm text-muted-foreground">League matches where {coach.name} is on the team sheet as coach, season by season. Real records, with the working shown; the provider can still have gaps or errors.</p>
    <p className="mt-2 text-xs text-muted-foreground">Provider coach ID {coach.apiId} · data pulled {date(snapshot.retrievedAt)}. Each club and season is kept separate. It is a snapshot, not a live feed, and says nothing about his current job.</p>
    <div className="mt-5 rounded-lg bg-muted/40 p-4">
      <h3 className="font-semibold">His record, season by season</h3>
      <p className="mt-2 text-sm text-muted-foreground">{periods.length} {periods.length === 1 ? 'spell' : 'spells'} on record. Each covers only the matches we can tie to him, not always the full season. Read every number next to its coverage — leagues, squads and sample sizes differ.</p>
    ...
    <div className="mt-5 space-y-4">{periods.map((period, index) => {
      ...
      return <details key={`${period.club}-${period.season}-${index}`} open={index === 0} className="match-period scroll-mt-24 rounded-lg border border-border p-4 print:break-inside-avoid">
        ...
        <p className="mt-3 text-xs text-muted-foreground">Missing matches are left out, never counted as zero. First substitution is averaged over {m.averageFirstSubstitutionMinute.coverage.denominator} games where he made one.</p>
        <h3 className="mt-5 text-sm font-semibold">Form across the spell</h3>
        ...
        <p className="mt-2 text-xs text-muted-foreground">Games 1–5, 6–20 and 21+ within this sample. Not always his first games in the job, and not proof of his impact on their own.</p>
        ...
        <p className="mt-1 text-xs text-muted-foreground">{coverage(m.formations)}. A starting shape is not the same as how the team plays.</p>
    ...
    <p className="mt-4 text-xs text-muted-foreground">Source: <a className="text-primary underline" href="https://www.api-football.com/documentation-v3" target="_blank" rel="noreferrer">API-Football fixtures, lineups, events and match statistics</a>. These are records, not a forecast of success.</p>
  </section>
}
